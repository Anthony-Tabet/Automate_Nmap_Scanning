...
def classify_with_gemini(results):
    try:
        ...
        prompt = f"Classify the following scan results as Completed, Incomplete, or False Positive Rich. Also, recommend the next scan type (e.g., -sS, -T2, etc.) based on your analysis:\n\n{results}"
        ...
        classification, next_scan = response.text.strip().split('\n')[0], response.text.strip().split('\n')[1]
...
def classify_with_ollama(results):
    try:
        ...
        classification, next_scan = response.message.content.strip().split('\n')[0], response.message.content.strip().split('\n')[1]
        print(f"Ollama classification: {classification}, Next scan suggestion: {next_scan}")
        return classification, next_scan
    except Exception as e:
...
def classify_scan(results):
    ...
    try:
        user_msg_input_class = client.chat.completions.create(
            model="gpt-4",
            messages=[{
                "role": "system", "content": "You are a system that classifies scan results as 'Completed', 'Incomplete', or 'False Positive Rich' and recommends the next scan type."},
...
def scan_with_fallback(target):
    ...
    scan_results_text = "\n".join([f"{result['IP']} {result['Port']} {result['State']}" for result in results])
